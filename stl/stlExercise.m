...
params.lambda = 1e-1;   % sparsity cost
...
%  You will need to whitened the patches e啊awith the zca2 function 
...
% optimize
opttheta = minFunc( @(theta) softICACost(theta, x, params), randTheta, options); % Use x or xw
% load opttheta;
...
numClasses  = 4; % doing 5-class digit recognition
...
theta=[theta, zeros(featureSize, 1)]; % expand theta to include the last class.
...
% and softmaxModel
%%% YOUR CODE HERE %%%
accuracy = multi_classifier_accuracy(theta,trainFeatures, trainLabels);
fprintf('Training accuracy: %2.1f%%\n', 100*accuracy);
accuracy = multi_classifier_accuracy(theta,testFeatures, testLabels);
